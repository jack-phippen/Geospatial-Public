...
1.  **Explore the use of `extract` from the `terra` package by running `?terra::extract`. (Note we need to specify `terra::` because 'extract' is a function name in multiple packages we may have loaded in our session).**
    ...
    **Then, use `extract()` to get the elevation (using the `co_elevation_crop` object created in the lesson) at each of the three points in `poudre_points_prj` ? (2 pts)**
    ...
    ```{r}
#?terra::extract
  poudre_points_elev <- extract(co_elevation_terra,poudre_points_prj)
  names(poudre_points_elev) <- c("Location", "Elevation(m)")
  
  #fix names 
  poudre_points_elev$Location <- c("Mishawaka", "Rustic", "Blue Lake Trailhead")
  
  kableExtra::kable(poudre_points_elev,caption = "Elevation (m) of Select Locations Along Poudre Highway")
...
tm_shape(ut_elevation)+
   tm_raster(col.scale = tm_scale_continuous(), 
            col.legend = tm_legend("Elevation (m)"))+
  tm_shape(ut_counties)+
  tm_borders() #tm borders is SEE THROUGH! 

# #could I do this out of order?
# tm_shape(ut_counties)+
#   tm_polygons()+
#   tm_shape(ut_elevation)+ 
#   tm_raster(col.scale = tm_scale_continuous(), 
...
#short answer: No. Must build layer by layer
```
...
ut_county <- ut_counties %>% filter(NAME == "Utah")

#convert elevation to right type
ut_elevation <- ut_elevation %>% rast()

#crop elevation layer to that county
ut_elevation_crop <- crop(ut_elevation,ut_county)

#quickly show that picture
tm_shape(ut_elevation_crop)+
   tm_raster(col.scale = tm_scale_continuous(), 
            col.legend = tm_legend("Elevation (m)"))+
  tm_shape(ut_county)+
  tm_borders()
