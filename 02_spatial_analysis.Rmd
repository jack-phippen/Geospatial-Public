...
```{r}
larimer_county <- counties %>% filter(NAME == "Larimer") #snag larimer county

#filter occ_prj points to larimer county
...
```{r}
#first let us try to find the average distance for each species via dplyr

# species_avg <- occ_larimer %>% 
#   group_by(Species) %>% 
#   mutate(num_river_dist_m = as.numeric(river_dist_m)) %>% 
#   summarise(avg_river_dist_m = mean(num_river_dist_m,na.rm=TRUE))
# 
# #make plot
# species_avg %>% 
#   ggplot(aes(x=Species,y=avg_river_dist_m))+
#   geom_bar(stat="identity")+
#   labs(title = "Comparison of Avg. Distance Species are Found from rivers in Larimer County",
#        y="Average Distance (m)")+
#   theme_minimal()

#could we do this as one whole chain?
occ_larimer %>% 
  group_by(Species) %>% 
  mutate(num_river_dist_m = as.numeric(river_dist_m)) %>% 
  summarise(avg_river_dist_m = mean(num_river_dist_m,na.rm=TRUE)) %>%
  ...
  geom_bar(stat="identity")+
  labs(title = "Comparison of Avg. Distance Species are Found from Rivers in Larimer County",
       y="Average Distance to Nearest River (m)")+
  theme_minimal()
  
```

Which species is, on average, found closest to a river? **Turns out, Elk!**
...
freq(landcover) %>% 
  ...
  geom_bar(stat="identity")+
  labs(x="Landcover Values/Types",
       y="Counts of Available Landcover")+
  coord_flip()+
  theme_minimal()
```
...
```{r}
classes <- as.data.frame(cats(landcover)) #coerce to a data frame because cats() 
#actually returns it as a list
```
...
```{r}
#we are going to perform a left_join bc we want to keep our values in the county
#landcover data frame. right join would leave us with the values in classes 
county_landcover <- left_join(county_landcover,classes) %>% 
  mutate(NLCD.Land.Cover.Class = as.factor(NLCD.Land.Cover.Class))

#next we want to map the counties each colored/filled by the most common NLCD 
#land cover class
tm_shape(county_landcover)+
 tm_fill("NLCD.Land.Cover.Class",
           col.legend = tm_legend("NLCD Land Cover Classes")) +
  tm_borders()
```
...
```{r}
crs(elevation)==crs(occ_prj) #crap, different CRS, let's fix that first

#convert elevation to right crs (note raster objects use project())
elevation_prj = project(elevation, crs(occ_prj))

crs(elevation_prj) == crs(occ_prj) #fixed!

#next, we need to figure out how to manipulate and extract values to answer the question
occ_elevation <- occ_prj %>% 
  mutate(elevation = extract(elevation_prj, occ_prj)) %>% 
  unnest(elevation) %>% 
  group_by(Species) %>%  #find average elevation for each species
  summarise(Avg_Elevation = mean(Elevation,na.rm=TRUE)) 

paste0("The species found at the highest elevation in CO. on average is the ",
       occ_elevation$Species[which.max(occ_elevation$Avg_Elevation)], 
       " with an average elevation of ",
       round(max(occ_elevation$Avg_Elevation),4),
       " meters.")
```
